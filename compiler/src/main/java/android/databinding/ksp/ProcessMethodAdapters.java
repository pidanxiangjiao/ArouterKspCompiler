...
public class ProcessMethodAdapters extends ProcessingStep {
    ...
    @Override
    public boolean onHandleKspStep(Resolver resolver, SymbolProcessorEnvironment processingEnvironment, CompilerArguments args) {
...
//    }
    ...
    private void clearIncrementalClasses(Resolver roundEnv, SetterStore store) {
        HashSet<String> classes = new HashSet<>();
        LibTypes libTypes = ModelAnalyzer.getInstance().libTypes;
        for (KSDeclaration element : AnnotationUtil.INSTANCE.getElementsAnnotatedWith(roundEnv, libTypes.getBindingAdapterClass().getName())) {
            KSDeclaration containingClass = element.getParentDeclaration();
